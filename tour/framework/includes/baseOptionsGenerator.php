 ...
 */
class pluginBaseOptionsGenerator {
	/**
	...
	function multiselect($item) {
		...
		if (!is_null($page)){
			...
			echo plugin_walk_page_multi_select_tree($pages,$depth,$args);
		}
		echo '</select>';
	...
	function get_select_target_options($type) {
		...
		switch($type){
			...
			case 'post':
				...
				break;
			case 'destination':
				$entries = get_posts('post_type=destination&orderby=title&numberposts=-1&order=ASC&suppress_filters=0');
				foreach($entries as $key => $entry) {
					$options[$entry->ID] = $entry->post_title;
				}
				break;
			...
			case 'slideshow_type':
				...
				if(theme_get_option('slideshow', 'revolution')){
					$options['revslider'] = __('Revolution Slider Plugin');
				}
 ...
 */
function plugin_walk_page_multi_select_tree() {
	$args = func_get_args();
	if ( empty($args[2]['walker']) ) // the user's options are the third parameter
		$walker = new Plugin_Walker_PageMultiSelect;
	else
 ...
 * @uses Walker
 */
class Plugin_Walker_PageMultiSelect extends Walker {
	/**
